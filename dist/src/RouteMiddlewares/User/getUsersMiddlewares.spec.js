...
describe(`getUsersValidationMiddleware`, () => {
  ...
  test("check that valid request passes", () => {
    ...
    const next = jest.fn();
    getUsersMiddlewares_1.retreiveUsersValidationMiddleware(
      request,
      response,
      next
    );
...
describe("setSearchQueryToLowerCaseMiddleware", () => {
  ...
  test("it should set userName to lowercase version of itself", () => {
    ...
    const next = jest.fn();
    getUsersMiddlewares_1.setSearchQueryToLowercaseMiddleware(
      request,
      response,
      next
    );
...
describe("getUsersByUsernameRegexSearchMiddleware", () => {
  test("that response.locals.users is populated and next is called", () =>
    __awaiter(this, void 0, void 0, function*() {
      ...
      const next = jest.fn();
      const middleware = getUsersMiddlewares_1.getRetreiveUsersByUsernameRegexSearchMiddleware(
        userModel
      );
  ...
  test("that next is called with err on failure", () =>
    __awaiter(this, void 0, void 0, function*() {
...
describe("formatUsersMiddleware", () => {
  test("checks that response.locals.formattedUsers contains a correctly formatted user", () => {
    ...
    const formattedUser = Object.assign({}, mockUser.toObject(), {
      password: undefined
    });
...
describe(`getUsersMiddlewares`, () => {
  test("that getUsersMiddlewares are defined in the correct order", () => {
    expect.assertions(5);
    expect(getUsersMiddlewares_1.getUsersMiddlewares[0]).toBe(
      getUsersMiddlewares_1.retreiveUsersValidationMiddleware
    );
    expect(getUsersMiddlewares_1.getUsersMiddlewares[1]).toBe(
      getUsersMiddlewares_1.setSearchQueryToLowercaseMiddleware
    );
    expect(getUsersMiddlewares_1.getUsersMiddlewares[2]).toBe(
      getUsersMiddlewares_1.retreiveUsersByUsernameRegexSearchMiddleware
    );
    expect(getUsersMiddlewares_1.getUsersMiddlewares[3]).toBe(
      getUsersMiddlewares_1.formatUsersMiddleware
    );
    expect(getUsersMiddlewares_1.getUsersMiddlewares[4]).toBe(
      getUsersMiddlewares_1.sendFormattedUsersMiddleware
    );
  });
});
