...
exports.retreiveUsersValidationMiddleware = (request, response, next) => {
    Joi.validate(request.query, getUsersValidationSchema, validationErrorMessageSenderMiddleware_1.getValidationErrorMessageSenderMiddleware(request, response, next));
};
...
exports.getRetreiveUsersByUsernameRegexSearchMiddleware = userModel => (request, response, next) => __awaiter(this, void 0, void 0, function* () {
    try {
        const { lowerCaseSearchQuery } = response.locals;
        response.locals.users = yield userModel.find({ userName: { $regex: lowerCaseSearchQuery } });
        next();
    }
    catch (err) {
        next(err);
...
exports.retreiveUsersByUsernameRegexSearchMiddleware = exports.getRetreiveUsersByUsernameRegexSearchMiddleware(User_1.userModel);
...
exports.sendFormattedUsersMiddleware = (request, response, next) => {
    try {
        ...
        return response.status(responseCodes_1.ResponseCodes.success).send({ users: formattedUsers });
    }
    catch (err) {
        next(err);
    }
