...
exports.loginRequestValidationMiddleware = (request, response, next) => {
    Joi.validate(request.body, loginValidationSchema, validationErrorMessageSenderMiddleware_1.getValidationErrorMessageSenderMiddleware(request, response, next));
};
exports.getRetreiveUserWithEmailMiddleware = userModel => (request, response, next) => __awaiter(this, void 0, void 0, function* () {
    try {
...
exports.retreiveUserWithEmailMiddleware = exports.getRetreiveUserWithEmailMiddleware(User_1.userModel);
...
const localisedUserDoesNotExistMessage = getLocalisedString_1.getLocalisedString(messageCategories_1.MessageCategories.failureMessages, failureMessages_1.FailureMessageKeys.loginUnsuccessfulMessage);
exports.userExistsValidationMiddleware = exports.getUserExistsValidationMiddleware(localisedUserDoesNotExistMessage);
exports.getCompareRequestPasswordToUserHashedPasswordMiddleware = compare => (request, response, next) => __awaiter(this, void 0, void 0, function* () {
    try {
        const requestPassword = request.body.password;
        const { password } = response.locals.user;
        response.locals.passwordMatchesHash = yield compare(requestPassword, password);
        next();
    }
    catch (err) {
        next(err);
...
exports.compareRequestPasswordToUserHashedPasswordMiddleware = exports.getCompareRequestPasswordToUserHashedPasswordMiddleware(bcryptjs_1.compare);
exports.getPasswordsMatchValidationMiddleware = loginUnsuccessfulMessage => (request, response, next) => {
    try {
...
const localisedLoginUnsuccessfulMessage = getLocalisedString_1.getLocalisedString(messageCategories_1.MessageCategories.failureMessages, failureMessages_1.FailureMessageKeys.loginUnsuccessfulMessage);
exports.passwordsMatchValidationMiddleware = exports.getPasswordsMatchValidationMiddleware(localisedLoginUnsuccessfulMessage);
...
exports.getSetJsonWebTokenExpiryInfoMiddleware = (expiresIn, unitOfTime) => (request, response, next) => {
    try {
...
exports.setJsonWebTokenExpiryInfoMiddleware = exports.getSetJsonWebTokenExpiryInfoMiddleware(oneMonthInSeconds, unitOfTime);
exports.getSetJsonWebTokenMiddleware = (signToken, jwtSecret) => (request, response, next) => {
    try {
...
exports.setJsonWebTokenMiddleware = exports.getSetJsonWebTokenMiddleware(jwt.sign, jwt_secret_1.jwtSecret);
exports.getLoginSuccessfulMiddleware = (loginSuccessMessage) => (request, response, next) => {
    try {
        const { jsonWebToken, expiry } = response.locals;
        return response.status(responseCodes_1.ResponseCodes.success).send({ jsonWebToken, message: loginSuccessMessage, expiry });
    }
    catch (err) {
        next(err);
    }
...
exports.loginSuccessfulMiddleware = exports.getLoginSuccessfulMiddleware(localisedLoginSuccessMessage);
