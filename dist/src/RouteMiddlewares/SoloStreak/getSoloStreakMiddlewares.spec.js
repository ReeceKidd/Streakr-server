...
describe(`getSoloStreakParamsValidationMiddleware`, () => {
    ...
    test("that next() is called when correct params are supplied", () => {
        ...
        const next = jest.fn();
        getSoloStreakMiddlewares_1.getSoloStreakParamsValidationMiddleware(request, response, next);
...
describe("retreiveSoloStreakMiddleware", () => {
    test("that response.locals.soloStreak is defined and next() is called", () => __awaiter(this, void 0, void 0, function* () {
        ...
        const next = jest.fn();
        const middleware = getSoloStreakMiddlewares_1.getRetreiveSoloStreakMiddleware(soloStreakModel);
...
describe("sendSoloStreakDoesNotExistErrorResponse", () => {
    test("that correct error response is sent whem soloStreak is not defined", () => {
        ...
        const next = jest.fn();
        const middleware = getSoloStreakMiddlewares_1.getSendSoloStreakDoesNotExistErrorMessageMiddleware(doesNotExistErrorResponseCode, localisedSoloStreakDoesNotExistMessage);
...
describe("sendSoloStreakMiddleware", () => {
    test("that soloStreak is sent correctly", () => {
        ...
        const middleware = getSoloStreakMiddlewares_1.getSendSoloStreakMiddleware(resourceCreatedCode);
        middleware(request, response, next);
        expect(next).not.toBeCalled();
    ...
    test("that on error next is called with error", () => __awaiter(this, void 0, void 0, function* () {
        ...
        const middleware = getSoloStreakMiddlewares_1.getSendSoloStreakMiddleware(resourceCreatedResponseCode);
        yield middleware(request, response, next);
        expect(next).toBeCalledWith(new TypeError("Cannot destructure property `soloStreak` of 'undefined' or 'null'."));
    }));
});
describe("getSoloStreakMiddlewares", () => {
    test("that getSoloStreakMiddlewares are defined in the correct order", () => {
        expect.assertions(5);
        expect(getSoloStreakMiddlewares_1.getSoloStreakMiddlewares.length).toEqual(4);
        expect(getSoloStreakMiddlewares_1.getSoloStreakMiddlewares[0]).toEqual(getSoloStreakMiddlewares_1.getSoloStreakParamsValidationMiddleware);
        expect(getSoloStreakMiddlewares_1.getSoloStreakMiddlewares[1]).toEqual(getSoloStreakMiddlewares_1.retreiveSoloStreakMiddleware);
        expect(getSoloStreakMiddlewares_1.getSoloStreakMiddlewares[2]).toEqual(getSoloStreakMiddlewares_1.sendSoloStreakDoesNotExistErrorMessageMiddleware);
        expect(getSoloStreakMiddlewares_1.getSoloStreakMiddlewares[3]).toEqual(getSoloStreakMiddlewares_1.sendSoloStreakMiddleware);
    });
});
