...
const AgendaJob_1 = require("../../Models/AgendaJob");
...
exports.soloStreakRegistrationValidationMiddleware = (request, response, next) => {
    Joi.validate(request.body, soloStreakRegisterstrationValidationSchema, validationErrorMessageSenderMiddleware_1.getValidationErrorMessageSenderMiddleware(request, response, next));
};
...
exports.retreiveTimezoneHeaderMiddleware = (request, response, next) => {
    try {
        response.locals.timezone = request.header(headers_1.SupportedRequestHeaders.xTimezone);
        next();
    }
    catch (err) {
        next(err);
...
exports.getSendMissingTimezoneErrorResponseMiddleware = localisedErrorMessage => (request, response, next) => {
    try {
        const { timezone } = response.locals;
        if (!timezone) {
            return response.status(responseCodes_1.ResponseCodes.unprocessableEntity).send({ message: localisedErrorMessage });
        }
        next();
    }
    catch (err) {
...
exports.sendMissingTimezoneErrorResponseMiddleware = exports.getSendMissingTimezoneErrorResponseMiddleware(localisedMissingTimezoneHeaderMessage);
...
exports.sendInvalidTimezoneErrorResponseMiddleware = exports.getSendInvalidTimezoneErrorResponseMiddleware(localisedInvalidTimezoneMessage);
exports.getDefineCurrentTimeMiddleware = moment => (request, response, next) => {
    try {
        const { timezone } = response.locals;
...
exports.defineStartDayMiddleware = exports.getDefineStartDayMiddleware(createSoloStreakCompleteTaskMiddlewares_1.dayFormat);
exports.getDefineEndOfDayMiddleware = (dayTimeRange) => (request, response, next) => {
    try {
        const { currentTime } = response.locals;
...
exports.defineEndOfDayMiddleware = exports.getDefineEndOfDayMiddleware(agenda_1.AgendaTimeRanges.day);
exports.getCreateSoloStreakFromRequestMiddleware = soloStreak => (request, response, next) => {
    try {
        const { timezone } = response.locals;
...
exports.createSoloStreakFromRequestMiddleware = exports.getCreateSoloStreakFromRequestMiddleware(SoloStreak_1.soloStreakModel);
exports.saveSoloStreakToDatabaseMiddleware = (request, response, next) => __awaiter(this, void 0, void 0, function* () {
    try {
...
exports.getDoesTimezoneAlreadyExistMiddleware = agendaJobModel => (request, response, next) => __awaiter(this, void 0, void 0, function* () {
    try {
        const { timezone } = response.locals;
        const doesTimezoneAlreadyExist = yield agendaJobModel.findOne({ "data.timezone": timezone });
...
exports.doesTimezoneAlreadyExistMiddleware = exports.getDoesTimezoneAlreadyExistMiddleware(AgendaJob_1.agendaJobModel);
exports.getCreateDailySoloStreakCompleteCheckerForTimezoneMiddleware = (agenda, soloStreakCompleteTrackerForTimezoneJobName) => (request, response, next) => __awaiter(this, void 0, void 0, function* () {
    try {
        const { endOfDay, timezone, doesTimezoneAlreadyExist } = response.locals;
        if (!doesTimezoneAlreadyExist) {
            yield agenda.start();
            const job = yield agenda.schedule(endOfDay, soloStreakCompleteTrackerForTimezoneJobName, { timezone });
            yield job.repeatEvery(agenda_1.AgendaProcessTimes.day);
            yield job.save();
        }
        next();
    }
...
exports.createDailySoloStreakCompleteCheckerForTimezoneMiddleware = exports.getCreateDailySoloStreakCompleteCheckerForTimezoneMiddleware(agenda_1.default, agenda_1.AgendaJobs.soloStreakCompleteForTimezoneTracker);
exports.sendFormattedSoloStreakMiddleware = (request, response, next) => {
    try {
        const { savedSoloStreak } = response.locals;
        return response.status(responseCodes_1.ResponseCodes.created).send(savedSoloStreak);
    }
    catch (err) {
        next(err);
    }
...
exports.createSoloStreakMiddlewares = [
    ...
    exports.doesTimezoneAlreadyExistMiddleware,
    exports.createDailySoloStreakCompleteCheckerForTimezoneMiddleware,
