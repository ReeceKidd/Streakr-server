...
describe("getSoloStreaksValidationMiddleware", () => {
  test("check that valid request passes", () => {
    ...
    const next = jest.fn();
    getSoloStreaksMiddlewares_1.getSoloStreaksValidationMiddleware(
      request,
      response,
      next
    );
...
describe("findSoloStreaksMiddleware", () => {
  test("check that soloStreaks are retreived correctly", () =>
    __awaiter(this, void 0, void 0, function*() {
      ...
      const next = jest.fn();
      const middleware = getSoloStreaksMiddlewares_1.getFindSoloStreaksMiddleware(
        soloStreakModel
      );
...
describe("sendSoloStreaksMiddleware", () => {
  test("should send soloStreaks in response", () => {
    ...
    const next = jest.fn();
    getSoloStreaksMiddlewares_1.sendSoloStreaksMiddleware(
      request,
      response,
      next
    );
...
describe(`getSoloStreaksMiddlewares`, () => {
  test("that getSoloStreaksMiddlewares are defined in the correct order", () =>
    __awaiter(this, void 0, void 0, function*() {
      expect.assertions(3);
      expect(getSoloStreaksMiddlewares_1.getSoloStreaksMiddlewares[0]).toBe(
        getSoloStreaksMiddlewares_1.getSoloStreaksValidationMiddleware
      );
      expect(getSoloStreaksMiddlewares_1.getSoloStreaksMiddlewares[1]).toBe(
        getSoloStreaksMiddlewares_1.findSoloStreaksMiddleware
      );
      expect(getSoloStreaksMiddlewares_1.getSoloStreaksMiddlewares[2]).toBe(
        getSoloStreaksMiddlewares_1.sendSoloStreaksMiddleware
      );
    }));
});
