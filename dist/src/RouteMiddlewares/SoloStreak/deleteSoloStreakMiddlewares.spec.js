...
describe("soloStreakParamsValidationMiddleware", () => {
    test("that correct response is sent when soloStreakId is not defined", () => {
        ...
        const next = jest.fn();
        deleteSoloStreakMiddlewares_1.soloStreakParamsValidationMiddleware(request, response, next);
        expect.assertions(3);
...
describe("deleteSoloStreakMiddleware", () => {
    test("that when soloStreak is deleted successfully response.locals.deletedSoloStreak is defined and next is called", () => __awaiter(this, void 0, void 0, function* () {
        expect.assertions(3);
        ...
        const next = jest.fn();
        const middleware = deleteSoloStreakMiddlewares_1.getDeleteSoloStreakMiddleware(soloStreakModel);
...
describe("soloStreakNotFoundMiddleware", () => {
    test("that correct error response is sent whem deletedSoloStreak is not defined", () => {
        ...
        const next = jest.fn();
        const middleware = deleteSoloStreakMiddlewares_1.getSoloStreakNotFoundMiddleware(badRequestResponseCode, localisedSoloStreakDoesNotExistMessage);
...
describe("sendSoloStreakDeletedResponseMiddleware", () => {
    test("that on deletion correct response is sent", () => {
        ...
        const next = jest.fn();
        const middleware = deleteSoloStreakMiddlewares_1.getSendSoloStreakDeletedResponseMiddleware(successfulDeletionResponseCode);
        middleware(request, response, next);
...
describe("deleteSoloStreakMiddlewares", () => {
    test("that deleteSoloStreakMiddlewares are defined in the correct order", () => {
        expect.assertions(5);
        expect(deleteSoloStreakMiddlewares_1.deleteSoloStreakMiddlewares.length).toEqual(4);
        expect(deleteSoloStreakMiddlewares_1.deleteSoloStreakMiddlewares[0]).toEqual(deleteSoloStreakMiddlewares_1.soloStreakParamsValidationMiddleware);
        expect(deleteSoloStreakMiddlewares_1.deleteSoloStreakMiddlewares[1]).toEqual(deleteSoloStreakMiddlewares_1.deleteSoloStreakMiddleware);
        expect(deleteSoloStreakMiddlewares_1.deleteSoloStreakMiddlewares[2]).toEqual(deleteSoloStreakMiddlewares_1.soloStreakNotFoundMiddleware);
        expect(deleteSoloStreakMiddlewares_1.deleteSoloStreakMiddlewares[3]).toEqual(deleteSoloStreakMiddlewares_1.sendSoloStreakDeletedResponseMiddleware);
    });
});
