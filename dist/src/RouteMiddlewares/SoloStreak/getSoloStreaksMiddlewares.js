...
})(GetSoloStreaksQueryParamaters = exports.GetSoloStreaksQueryParamaters || (exports.GetSoloStreaksQueryParamaters = {}));
const getSoloStreaksValidationSchema = {
    userId: Joi.string().required()
};
exports.getSoloStreaksValidationMiddleware = (request, response, next) => {
    Joi.validate(request.query, getSoloStreaksValidationSchema, validationErrorMessageSenderMiddleware_1.getValidationErrorMessageSenderMiddleware(request, response, next));
};
exports.getFindSoloStreaksMiddleware = soloStreakModel => (request, response, next) => __awaiter(this, void 0, void 0, function* () {
    try {
...
exports.findSoloStreaksMiddleware = exports.getFindSoloStreaksMiddleware(SoloStreak_1.soloStreakModel);
exports.sendSoloStreaksMiddleware = (request, response, next) => {
    try {
        const { soloStreaks } = response.locals;
        response.status(responseCodes_1.ResponseCodes.success).send({ soloStreaks });
    }
    catch (err) {
        next(err);
    }
