...
describe(`getFriendsValidationMiddleware`, () => {
  test("check that valid request passes", () => {
    ...
    const next = jest.fn();
    getFriendsMiddlewares_1.getFriendsValidationMiddleware(
      request,
      response,
      next
    );
...
describe(`retreiveUserMiddleware`, () => {
  ...
  test("should define response.locals.user when user is found", () =>
    __awaiter(this, void 0, void 0, function*() {
      ...
      const next = jest.fn();
      const middleware = getFriendsMiddlewares_1.getRetreiveUserMiddleware(
        UserModel
      );
  ...
  test("should send error response when user doesn't exist", () =>
    __awaiter(this, void 0, void 0, function*() {
      expect.assertions(3);
...
describe(`userExistsValidationMiddleware`, () => {
  ...
  test("check that error response is returned correctly when user wasn't found", () =>
    __awaiter(this, void 0, void 0, function*() {
      ...
      const next = jest.fn();
      const middleware = getFriendsMiddlewares_1.getUserExistsValidationMiddleware(
        mockErrorMessage
      );
...
describe("getRetreiveFriendsMiddleware", () => {
  test("check that friends are retreived correctly", () =>
    __awaiter(this, void 0, void 0, function*() {
      expect.assertions(3);
      const findOne = jest.fn(() => Promise.resolve(true));
      const UserModel = {
      ...
      const request = {};
      const response = {
        locals: { user: { friends: ["123", "124", "125"] } }
      };
      const next = jest.fn();
      const middleware = getFriendsMiddlewares_1.getRetreiveFriendsMiddleware(
        UserModel
      );
      yield middleware(request, response, next);
  ...
  test("checks that next is called with error if database call fails", () =>
    __awaiter(this, void 0, void 0, function*() {
      expect.assertions(2);
      const ERROR_MESSAGE = "error";
      const findOne = jest.fn(() => Promise.reject(ERROR_MESSAGE));
...
describe("sendFormattedFriendsMiddleware", () => {
  test("should send formattedFriends in response", () => {
    ...
    const next = jest.fn();
    getFriendsMiddlewares_1.sendFormattedFriendsMiddleware(
      request,
      response,
      next
    );
...
describe(`getFriendsMiddlewares`, () => {
  test("that getFriendsMiddlewares are defined in the correct order", () => {
    expect.assertions(6);
    expect(getFriendsMiddlewares_1.getFriendsMiddlewares[0]).toBe(
      getFriendsMiddlewares_1.getFriendsValidationMiddleware
    );
    expect(getFriendsMiddlewares_1.getFriendsMiddlewares[1]).toBe(
      getFriendsMiddlewares_1.retreiveUserMiddleware
    );
    expect(getFriendsMiddlewares_1.getFriendsMiddlewares[2]).toBe(
      getFriendsMiddlewares_1.userExistsValidationMiddleware
    );
    expect(getFriendsMiddlewares_1.getFriendsMiddlewares[3]).toBe(
      getFriendsMiddlewares_1.retreiveFriendsMiddleware
    );
    expect(getFriendsMiddlewares_1.getFriendsMiddlewares[4]).toBe(
      getFriendsMiddlewares_1.formatFriendsMiddleware
    );
    expect(getFriendsMiddlewares_1.getFriendsMiddlewares[5]).toBe(
      getFriendsMiddlewares_1.sendFormattedFriendsMiddleware
    );
  });
});
