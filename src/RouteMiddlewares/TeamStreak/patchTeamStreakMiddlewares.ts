...
export const getPatchTeamStreakMiddleware = (
...
export const patchTeamStreakMiddleware = getPatchTeamStreakMiddleware(
  teamStreakModel
);

export const sendUpdatedTeamStreakMiddleware = (
  request: Request,
  response: Response,
  next: NextFunction
) => {
  try {
    ...
    return response.status(ResponseCodes.success).send(updatedTeamStreak);
  } catch (err) {
