...
describe("retreiveTeamStreakMiddleware", () => {
  ...
  test("throws GetTeamStreakNoTeamStreakFound when group streak is not found", async () => {
    ...
    expect(next).toBeCalledWith(
      new CustomError(ErrorType.GetTeamStreakNoTeamStreakFound)
    );
  });
  ...
  test("calls next with RetreiveTeamStreakMiddleware error on middleware failure", async () => {
    ...
    expect(next).toBeCalledWith(
      new CustomError(ErrorType.RetreiveTeamStreakMiddleware, expect.any(Error))
    );
  });
});
...
describe("retreiveTeamStreakMembersInformation", () => {
  test("retreives group streak members information and sets response.locals.TeamStreaksWithUsers", async () => {
    ...
    const middleware = getRetreiveTeamStreakMembersInformationMiddleware(
      userModel,
      teamStreakModel
    );
    await middleware(request, response, next);
...
describe("retreiveTeamStreakCreatorInformation", () => {
  test("retreives group streak creator information and sets response.locals.teamStreak", async () => {
    ...
    const middleware = getRetreiveTeamStreakCreatorInformationMiddleware(
      userModel
    );
    await middleware(request, response, next);
...
describe("sendTeamStreakMiddleware", () => {
  ...
  test("calls next with SendTeamStreakMiddleware error on middleware failure", async () => {
    ...
    expect(next).toBeCalledWith(
      new CustomError(ErrorType.SendTeamStreakMiddleware, expect.any(Error))
    );
  });
});
...
describe("getTeamStreakMiddlewares", () => {
  test("that getTeamStreakMiddlewares are defined in the correct order", () => {
    ...
    expect(getOneTeamStreakMiddlewares[0]).toEqual(
      getTeamStreakParamsValidationMiddleware
    );
    expect(getOneTeamStreakMiddlewares[1]).toEqual(
      retreiveTeamStreakMiddleware
    );
    expect(getOneTeamStreakMiddlewares[2]).toEqual(
      retreiveTeamStreakMembersInformationMiddleware
    );
    expect(getOneTeamStreakMiddlewares[3]).toEqual(
      retreiveTeamStreakCreatorInformationMiddleware
    );
