...
export const getDeleteTeamStreakMiddleware = (teamStreakModel: mongoose.Model<TeamStreakModel>) => async (
    request: Request,
    response: Response,
    next: NextFunction,
): Promise<void> => {
    try {
        ...
        const deletedTeamStreak = await teamStreakModel.findByIdAndDelete(teamStreakId);
...
export const deleteTeamStreakMiddleware = getDeleteTeamStreakMiddleware(teamStreakModel);

export const getSendTeamStreakDeletedResponseMiddleware = (successfulDeletetionResponseCode: ResponseCodes) => (
    request: Request,
    response: Response,
    next: NextFunction,
): void => {
    try {
        ...
        next(new CustomError(ErrorType.SendTeamStreakDeletedResponseMiddleware, err));
    }
};
