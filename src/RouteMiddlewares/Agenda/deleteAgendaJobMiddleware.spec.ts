...
describe('agendaJobParamsValidationMiddleware', () => {
    ...
    test('sends agendaJobId is not a string error', () => {
        ...
        expect(status).toHaveBeenCalledWith(ResponseCodes.unprocessableEntity);
        expect(send).toBeCalledWith({
            message: 'child "agendaJobId" fails because ["agendaJobId" must be a string]',
        });
        expect(next).not.toBeCalled();
    });
});
...
describe('deleteAgendaJobMiddleware', () => {
    ...
    test('throws NoAgendaJobToDeleteFound error when no solo streak is found', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.NoAgendaJobToDeleteFound));
    });
    ...
    test('calls next with DeleteAgendaJobMiddleware error on failure', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.DeleteAgendaJobMiddleware, expect.any(Error)));
    });
});
...
describe('sendAgendaJobDeletedResponseMiddleware', () => {
    test('responds with successful deletion', () => {
        ...
        const next = jest.fn();
        const middleware = getSendAgendaJobDeletedResponseMiddleware(successfulDeletionResponseCode);
    ...
    test('that on error next is called with error', () => {
        ...
        expect(next).toBeCalledWith(
            new CustomError(ErrorType.SendAgendaJobDeletedResponseMiddleware, expect.any(Error)),
        );
    });
});
...
describe('deleteAgendaJobMiddlewares', () => {
    test('that deleteAgendaJobMiddlewares are defined in the correct order', () => {
        ...
        expect(deleteAgendaJobMiddlewares[0]).toEqual(agendaJobParamsValidationMiddleware);
        expect(deleteAgendaJobMiddlewares[1]).toEqual(deleteAgendaJobMiddleware);
        expect(deleteAgendaJobMiddlewares[2]).toEqual(sendAgendaJobDeletedResponseMiddleware);
    });
});
