...
export const getFindGroupMemberStreaksMiddleware = (
    ...
    try {
        ...
        response.locals.groupMemberStreaks = await groupMemberStreakModel.find(query);
        next();
    } catch (err) {
...
export const findGroupMemberStreaksMiddleware = getFindGroupMemberStreaksMiddleware(groupMemberStreakModel);

export const sendGroupMemberStreaksMiddleware = (request: Request, response: Response, next: NextFunction): void => {
    try {
