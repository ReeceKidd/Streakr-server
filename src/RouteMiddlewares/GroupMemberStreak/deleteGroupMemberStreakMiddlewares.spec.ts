...
describe('deleteGroupMemberStreakMiddleware', () => {
    test('sets response.locals.deletedGroupMemberStreak', async () => {
        ...
        const next = jest.fn();
        const middleware = getDeleteGroupMemberStreakMiddleware(groupMemberStreakModel as any);
    ...
    test('throws NoGroupMemberStreakToDeleteFound error when no solo streak is found', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.NoGroupMemberStreakToDeleteFound));
    });
    ...
    test('calls next with DeleteGroupMemberStreakMiddleware error on failure', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.DeleteGroupMemberStreakMiddleware, expect.any(Error)));
    });
});
...
describe('sendGroupMemberStreakDeletedResponseMiddleware', () => {
    test('responds with successful deletion', () => {
        ...
        const next = jest.fn();
        const middleware = getSendGroupMemberStreakDeletedResponseMiddleware(successfulDeletionResponseCode);
    ...
    test('that on error next is called with error', () => {
        ...
        expect(next).toBeCalledWith(
            new CustomError(ErrorType.SendGroupMemberStreakDeletedResponseMiddleware, expect.any(Error)),
        );
    });
});
...
describe('deleteGroupMemberStreakMiddlewares', () => {
    test('that deleteGroupMemberStreakMiddlewares are defined in the correct order', () => {
        ...
        expect(deleteGroupMemberStreakMiddlewares[0]).toEqual(groupMemberStreakParamsValidationMiddleware);
        expect(deleteGroupMemberStreakMiddlewares[1]).toEqual(deleteGroupMemberStreakMiddleware);
        expect(deleteGroupMemberStreakMiddlewares[2]).toEqual(sendGroupMemberStreakDeletedResponseMiddleware);
    });
});
