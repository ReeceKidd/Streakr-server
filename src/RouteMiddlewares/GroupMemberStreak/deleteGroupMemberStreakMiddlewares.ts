...
export const getDeleteGroupMemberStreakMiddleware = (
  ...
  try {
    ...
    const deletedGroupMemberStreak = await groupMemberStreakModel.findByIdAndDelete(
      groupMemberStreakId
    );
    ...
    if (err instanceof CustomError) next(err);
      ...
      next(new CustomError(ErrorType.DeleteGroupMemberStreakMiddleware, err));
  }
};

export const deleteGroupMemberStreakMiddleware = getDeleteGroupMemberStreakMiddleware(
  groupMemberStreakModel
);

export const getSendGroupMemberStreakDeletedResponseMiddleware = (
