...
export const getRetreiveUserMiddleware = (userModel: mongoose.Model<UserModel>) => async (
    ...
    try {
        ...
        else next(new CustomError(ErrorType.CreateGroupMemberStreakRetreiveUserMiddleware, err));
    }
};
...
export const getRetreiveTeamStreakMiddleware = (teamStreakModel: mongoose.Model<TeamStreakModel>) => async (
    ...
    try {
        ...
        if (!teamStreak) {
            throw new CustomError(ErrorType.CreateGroupMemberStreakTeamStreakDoesNotExist);
        }
        response.locals.teamStreak = teamStreak;
        next();
    } catch (err) {
        if (err instanceof CustomError) next(err);
        else next(new CustomError(ErrorType.CreateGroupMemberStreakRetreiveTeamStreakMiddleware, err));
    }
};
...
export const getCreateGroupMemberStreakFromRequestMiddleware = (
    ...
    try {
        ...
        next(new CustomError(ErrorType.CreateGroupMemberStreakFromRequestMiddleware, err));
    }
};
...
export const saveGroupMemberStreakToDatabaseMiddleware = async (
    ...
    try {
        const newGroupMemberStreak: GroupMemberStreakModel = response.locals.newGroupMemberStreak;
        response.locals.savedGroupMemberStreak = await newGroupMemberStreak.save();
        next();
    } catch (err) {
        next(new CustomError(ErrorType.SaveGroupMemberStreakToDatabaseMiddleware, err));
    }
};
...
export const sendFormattedGroupMemberStreakMiddleware = (
    ...
    try {
        ...
        response.status(ResponseCodes.created).send(savedGroupMemberStreak);
    } catch (err) {
        next(new CustomError(ErrorType.SendFormattedGroupMemberStreakMiddleware, err));
    }
};
