...
describe('findGroupMemberStreaksMiddleware', () => {
    test('queries database with just userId and sets response.locals.groupMemberStreaks', async () => {
        ...
        const next = jest.fn();
        const middleware = getFindGroupMemberStreaksMiddleware(groupMemberStreakModel as any);
    ...
    test('calls next with FindGroupMemberStreaksMiddleware error on middleware failure', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.FindGroupMemberStreaksMiddleware, expect.any(Error)));
    });
});
...
describe('sendGroupMemberStreaksMiddleware', () => {
    ...
    test('calls next with SendGroupMemberStreaksMiddleware on middleware failure', () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.SendGroupMemberStreaksMiddleware, expect.any(Error)));
    });
});
...
describe(`getAllGroupMemberStreaksMiddlewares`, () => {
    test('that getGroupMemberStreaksMiddlewares are defined in the correct order', async () => {
        ...
        expect(getAllGroupMemberStreaksMiddlewares[0]).toBe(getGroupMemberStreaksQueryValidationMiddleware);
        expect(getAllGroupMemberStreaksMiddlewares[1]).toBe(findGroupMemberStreaksMiddleware);
        expect(getAllGroupMemberStreaksMiddlewares[2]).toBe(sendGroupMemberStreaksMiddleware);
    });
});
