...
describe('sendStreakTrackingEventMiddleware', () => {
    test('sends streakTrackingEvent', () => {
        ...
        const middleware = getSendStreakTrackingEventMiddleware(resourceCreatedCode);
    ...
    test('calls next with SendStreakTrackingEventMiddleware error on middleware failure', async () => {
        ...
        const middleware = getSendStreakTrackingEventMiddleware(resourceCreatedResponseCode);

        await middleware(request, response, next);

        expect(next).toBeCalledWith(new CustomError(ErrorType.SendStreakTrackingEventMiddleware, expect.any(Error)));
    });
});
