...
describe('streakTrackingEventParamsValidationMiddleware', () => {
    test('sends streakTrackingEventId is not defined error', () => {
        ...
        expect(send).toBeCalledWith({
            message: 'child "streakTrackingEventId" fails because ["streakTrackingEventId" is required]',
        });
    ...
    test('sends streakTrackingEventId is not a string error', () => {
        ...
        expect(status).toHaveBeenCalledWith(ResponseCodes.unprocessableEntity);
        expect(send).toBeCalledWith({
            message: 'child "streakTrackingEventId" fails because ["streakTrackingEventId" must be a string]',
        });
        expect(next).not.toBeCalled();
    });
});
...
describe('deleteStreakTrackingEventMiddleware', () => {
    test('sets response.locals.deletedStreakTrackingEvent', async () => {
        ...
        const next = jest.fn();
        const middleware = getDeleteStreakTrackingEventMiddleware(streakTrackingEventModel as any);
    ...
    test('throws NoStreakTrackingEventToDeleteFound error when no solo streak is found', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.NoStreakTrackingEventToDeleteFound));
    });
    ...
    test('calls next with DeleteStreakTrackingEventMiddleware error on failure', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.DeleteStreakTrackingEventMiddleware, expect.any(Error)));
    });
});
...
describe('sendStreakTrackingEventDeletedResponseMiddleware', () => {
    test('responds with successful deletion', () => {
        ...
        const next = jest.fn();
        const middleware = getSendStreakTrackingEventDeletedResponseMiddleware(successfulDeletionResponseCode);
    ...
    test('that on error next is called with error', () => {
        ...
        expect(next).toBeCalledWith(
            new CustomError(ErrorType.SendStreakTrackingEventDeletedResponseMiddleware, expect.any(Error)),
        );
    });
});
...
describe('deleteStreakTrackingEventMiddlewares', () => {
    test('that deleteStreakTrackingEventMiddlewares are defined in the correct order', () => {
        ...
        expect(deleteStreakTrackingEventMiddlewares[0]).toEqual(streakTrackingEventParamsValidationMiddleware);
        expect(deleteStreakTrackingEventMiddlewares[1]).toEqual(deleteStreakTrackingEventMiddleware);
        expect(deleteStreakTrackingEventMiddlewares[2]).toEqual(sendStreakTrackingEventDeletedResponseMiddleware);
    });
});
