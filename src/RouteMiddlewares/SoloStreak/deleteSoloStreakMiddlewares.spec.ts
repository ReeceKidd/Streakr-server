import {
  ...
  getSoloStreakNotFoundMiddleware,
...
describe("soloStreakNotFoundMiddleware", () => {
  test("that correct error response is sent whem deletedSoloStreak is not defined", () => {
    ...
    const next = jest.fn();
    const middleware = getSoloStreakNotFoundMiddleware(
...
describe("deleteSoloStreakMiddlewares", () => {
  test("that deleteSoloStreakMiddlewares are defined in the correct order", () => {
    ...
    expect(deleteSoloStreakMiddlewares[2]).toEqual(
      soloStreakNotFoundMiddleware
    );
    expect(deleteSoloStreakMiddlewares[3]).toEqual(
      sendSoloStreakDeletedResponseMiddleware
    );
  });
});
