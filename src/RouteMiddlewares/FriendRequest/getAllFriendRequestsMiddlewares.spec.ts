...
describe('getFriendRequestsValidationMiddleware', () => {
    ...
    test('sends requesterId cannot be a number error', () => {
        ...
        expect(send).toBeCalledWith({
            message: 'child "requesterId" fails because ["requesterId" must be a string]',
        });
    ...
    test('sends requesteeId cannot be a number error', () => {
        ...
        expect(status).toHaveBeenCalledWith(ResponseCodes.unprocessableEntity);
        expect(send).toBeCalledWith({
            message: 'child "requesteeId" fails because ["requesteeId" must be a string]',
        });
        expect(next).not.toBeCalled();
    });
...
describe('findFriendRequestsMiddleware', () => {
    test('queries database with just requesterId and sets response.locals.friendRequests', async () => {
        ...
        const next = jest.fn();
        const middleware = getFindFriendRequestsMiddleware(friendRequestModel as any);
    ...
    test('calls next with FindFriendRequestsMiddleware error on middleware failure', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.FindFriendRequestsMiddleware, expect.any(Error)));
    });
});
...
describe('populateFriendRequestsMiddleware', () => {
    test('sets response.locals.populatedFriendRequests and calls next', async () => {
        ...
        const next = jest.fn();
        const populateFriendRequestsMiddleware = getPopulateFriendRequestsMiddleware(userModel as any);
    ...
    test('calls next with PopulateFriendRequestsMiddleware error on middleware failure', async () => {
        ...
        const response: any = {};
        const next = jest.fn();
        const middleware = getPopulateFriendRequestsMiddleware(friendRequestModel as any);

        await middleware(request, response, next);

        expect(next).toBeCalledWith(new CustomError(ErrorType.PopulateFriendRequestsMiddleware, expect.any(Error)));
    });
});
...
describe('sendPopulatedFriendRequestsMiddleware', () => {
    ...
    test('calls next with SendFriendRequestsMiddleware on middleware failure', () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.SendFriendRequestsMiddleware, expect.any(Error)));
    });
});
...
describe(`getAllFriendRequestsMiddlewares`, () => {
    test('are in the correct order', async () => {
        ...
        expect(getAllFriendRequestsMiddlewares[0]).toBe(getFriendRequestsQueryValidationMiddleware);
        expect(getAllFriendRequestsMiddlewares[1]).toBe(findFriendRequestsMiddleware);
        expect(getAllFriendRequestsMiddlewares[2]).toBe(populateFriendRequestsMiddleware);
        expect(getAllFriendRequestsMiddlewares[3]).toBe(sendPopulatedFriendRequestsMiddleware);
    });
});
