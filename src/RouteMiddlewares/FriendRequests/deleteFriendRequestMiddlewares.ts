...
export const getDeleteFriendRequestMiddleware = (friendRequestModel: mongoose.Model<FriendRequestModel>) => async (
    request: Request,
    response: Response,
    next: NextFunction,
): Promise<void> => {
...
export const deleteFriendRequestMiddleware = getDeleteFriendRequestMiddleware(friendRequestModel);

export const getSendFriendRequestDeletedResponseMiddleware = (successfulDeletetionResponseCode: ResponseCodes) => (
    request: Request,
    response: Response,
    next: NextFunction,
): void => {
    try {
        ...
        next(new CustomError(ErrorType.SendFriendRequestDeletedResponseMiddleware, err));
    }
};
