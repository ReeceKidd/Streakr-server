...
export const userRequestBodyValidationMiddleware = (
  request: Request,
  response: Response,
  next: NextFunction
) => {
...
export const getPatchUserMiddleware = (
...
export const sendUpdatedUserMiddleware = (
  request: Request,
  response: Response,
  next: NextFunction
) => {
  try {
    ...
    return response.status(ResponseCodes.success).send(updatedUser);
  } catch (err) {
