...
import { FriendRequestStatus, FriendRequest, User, Friend } from '@streakoid/streakoid-sdk/lib';
...
export const addFriendParamsValidationMiddleware = (request: Request, response: Response, next: NextFunction): void => {
    Joi.validate(
        request.params,
...
export const addFriendBodyValidationMiddleware = (request: Request, response: Response, next: NextFunction): void => {
    Joi.validate(
        request.body,
...
export const isAlreadyAFriendMiddleware = (request: Request, response: Response, next: NextFunction): void => {
    try {
        const user: User = response.locals.user;
        const friendId: string = request.body.friendId;

        const isExistingFriend = user.friends.find(friend => friend.friendId == friendId);
        if (isExistingFriend) {
...
export const getRetreiveFriendMiddleware = (userModel: Model<UserModel>) => async (
    request: Request,
    response: Response,
    next: NextFunction,
): Promise<void> => {
...
export const getRetreiveFriendRequestMiddleware = (friendRequestModel: Model<FriendRequestModel>) => async (
    request: Request,
    response: Response,
    next: NextFunction,
): Promise<void> => {
...
export const retreiveFriendRequestMiddleware = getRetreiveFriendRequestMiddleware(friendRequestModel);

export const getAddFriendToUsersFriendListMiddleware = (userModel: Model<UserModel>) => async (
    request: Request,
    response: Response,
    next: NextFunction,
): Promise<void> => {
    try {
        ...
        if (!userWithNewFriend) {
            throw new CustomError(ErrorType.AddFriendToUsersFriendListNotFound);
        }
...
export const addFriendToUsersFriendListMiddleware = getAddFriendToUsersFriendListMiddleware(userModel);

export const getAddUserToFriendsFriendListMiddleware = (userModel: Model<UserModel>) => async (
    request: Request,
    response: Response,
    next: NextFunction,
): Promise<void> => {
...
export const addUserToFriendsFriendListMiddleware = getAddUserToFriendsFriendListMiddleware(userModel);

export const getUpdateFriendRequestStatusMiddleware = (friendRequestModel: Model<FriendRequestModel>) => async (
    request: Request,
    response: Response,
    next: NextFunction,
): Promise<void> => {
...
export const updateFriendRequestStatusMiddleware = getUpdateFriendRequestStatusMiddleware(friendRequestModel);

export const sendUserWithNewFriendMiddleware = (request: Request, response: Response, next: NextFunction): void => {
    try {
        const { updatedFriends } = response.locals;
        response.status(ResponseCodes.created).send(updatedFriends);
    } catch (err) {
