...
export const getDeleteCompleteGroupMemberStreakTaskMiddleware = (
    ...
    try {
        ...
        if (!deletedCompleteGroupMemberStreakTask) {
            throw new CustomError(ErrorType.NoCompleteGroupMemberStreakTaskToDeleteFound);
        }
        response.locals.deletedCompleteGroupMemberStreakTask = deletedCompleteGroupMemberStreakTask;
        next();
    } catch (err) {
        if (err instanceof CustomError) next(err);
        else next(new CustomError(ErrorType.DeleteCompleteGroupMemberStreakTaskMiddleware, err));
    }
};
...
export const getSendCompleteGroupMemberStreakTaskDeletedResponseMiddleware = (
    ...
    try {
        ...
        next(new CustomError(ErrorType.SendCompleteGroupMemberStreakTaskDeletedResponseMiddleware, err));
    }
};
