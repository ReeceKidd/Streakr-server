...
export const getFriendExistsMiddleware = (userModel: mongoose.Model<UserModel>) => async (
    ...
    try {
        ...
        else next(new CustomError(ErrorType.CreateGroupMemberFriendExistsMiddleware, err));
    }
};
...
export const getTeamStreakExistsMiddleware = (teamStreakModel: mongoose.Model<TeamStreakModel>) => async (
    ...
    try {
        ...
        if (err instanceof CustomError) next(err);
        else next(new CustomError(ErrorType.CreateGroupMemberTeamStreakExistsMiddleware, err));
    }
};
...
export const getCreateGroupMemberStreakMiddleware = (
    ...
    try {
        ...
        next(new CustomError(ErrorType.CreateGroupMemberCreateGroupMemberStreakMiddleware, err));
    }
};

export const createGroupMemberStreakMiddleware = getCreateGroupMemberStreakMiddleware(groupMemberStreakModel);
...
export const getAddFriendToTeamStreakMiddleware = (teamStreakModel: mongoose.Model<TeamStreakModel>) => async (
    ...
    try {
        ...
        members = [...members, { memberId: friendId, groupMemberStreakId: groupMemberStreak._id }];
...
export const sendCreateGroupMemberResponseMiddleware = (
    ...
    try {
        ...
        next(new CustomError(ErrorType.SendCreateGroupMemberResponseMiddleware, err));
    }
};
