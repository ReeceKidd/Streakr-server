...
describe('retreiveTeamStreakMiddleware', () => {
    ...
    test('throws DeleteGroupMemberRetreiveTeamStreakMiddleware error on middleware failure', async () => {
        ...
        expect(next).toBeCalledWith(
            new CustomError(ErrorType.DeleteGroupMemberRetreiveTeamStreakMiddleware, expect.any(Error)),
        );
    });
});
...
describe('deleteGroupMemberMiddleware', () => {
    ...
    test('calls next with DeleteGroupMemberMiddleware error on failure', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.DeleteGroupMemberMiddleware, expect.any(Error)));
    });
});
...
describe('sendGroupMemberDeletedResponseMiddleware', () => {
    ...
    test('that on error next is called with error', () => {
        ...
        expect(next).toBeCalledWith(
            new CustomError(ErrorType.SendGroupMemberDeletedResponseMiddleware, expect.any(Error)),
        );
    });
});
...
describe('deleteGroupMemberMiddlewares', () => {
    test('are defined in the correct order', () => {
        ...
        expect(deleteGroupMemberMiddlewares[0]).toEqual(groupMemberParamsValidationMiddleware);
        expect(deleteGroupMemberMiddlewares[1]).toEqual(retreiveTeamStreakMiddleware);
        expect(deleteGroupMemberMiddlewares[2]).toEqual(retreiveGroupMemberMiddleware);
        expect(deleteGroupMemberMiddlewares[3]).toEqual(deleteGroupMemberMiddleware);
        expect(deleteGroupMemberMiddlewares[4]).toEqual(sendGroupMemberDeletedResponseMiddleware);
    });
});
