...
describe('feedbackParamsValidationMiddleware', () => {
    ...
    test('sends feedbackId is not a string error', () => {
        ...
        expect(status).toHaveBeenCalledWith(ResponseCodes.unprocessableEntity);
        expect(send).toBeCalledWith({
            message: 'child "feedbackId" fails because ["feedbackId" must be a string]',
        });
        expect(next).not.toBeCalled();
    });
});
...
describe('deleteFeedbackMiddleware', () => {
    ...
    test('throws NoFeedbackToDeleteFound error when no solo streak is found', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.NoFeedbackToDeleteFound));
    });
    ...
    test('calls next with DeleteFeedbackMiddleware error on failure', async () => {
        ...
        expect(next).toBeCalledWith(new CustomError(ErrorType.DeleteFeedbackMiddleware, expect.any(Error)));
    });
});
...
describe('sendFeedbackDeletedResponseMiddleware', () => {
    test('responds with successful deletion', () => {
        ...
        const next = jest.fn();
        const middleware = getSendFeedbackDeletedResponseMiddleware(successfulDeletionResponseCode);
    ...
    test('that on error next is called with error', () => {
        ...
        expect(next).toBeCalledWith(
            new CustomError(ErrorType.SendFeedbackDeletedResponseMiddleware, expect.any(Error)),
        );
    });
});
...
describe('deleteFeedbackMiddlewares', () => {
    test('that deleteFeedbackMiddlewares are defined in the correct order', () => {
        ...
        expect(deleteFeedbackMiddlewares[0]).toEqual(feedbackParamsValidationMiddleware);
        expect(deleteFeedbackMiddlewares[1]).toEqual(deleteFeedbackMiddleware);
        expect(deleteFeedbackMiddlewares[2]).toEqual(sendFeedbackDeletedResponseMiddleware);
    });
});
