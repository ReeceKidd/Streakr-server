...
export const getCreateFeedbackFromRequestMiddleware = (feedbackModel: mongoose.Model<FeedbackModel>) => (
    request: Request,
    response: Response,
    next: NextFunction,
): void => {
...
export const createFeedbackFromRequestMiddleware = getCreateFeedbackFromRequestMiddleware(feedbackModel);
...
export const sendFormattedFeedbackMiddleware = (request: Request, response: Response, next: NextFunction): void => {
    try {
        const { savedFeedback } = response.locals;
        response.status(ResponseCodes.created).send(savedFeedback);
    } catch (err) {
