...
// export const initialiseGroupMemberStreakTimezoneCheckerJobs = async (): Promise<void> => {
...
//     });
//     console.log(`Number of group member streak daily tracker jobs: ${numberOfGroupMemberStreakTimezoneCheckerJobs}`);
...
// export const createGroupMemberStreakDailyTrackerJob = async (timezone: string) => {
//     const endOfDay = moment.tz(timezone).endOf(AgendaTimeRanges.day);
//     return (async () => {
//         const groupMemberStreakDailyTrackerJob = agenda.create(AgendaJobs.groupMemberStreakDailyTracker, { timezone });
//         groupMemberStreakDailyTrackerJob.schedule(endOfDay.toDate());
//         await agenda.start();
//         await groupMemberStreakDailyTrackerJob.repeatEvery(AgendaProcessTimes.oneDay).save();
