...
describe("resetSoloStreaksNotCompletedTodayByTimezone", () => {
    test("that function calls functions it requires correctly", async () => {
        ...
        const getIncompleteSoloStreaks = jest.fn(() => Promise.resolve(incompleteSoloStreaks));
        ...
        expect(resetIncompleteSoloStreaks).toBeCalledWith(soloStreakModel, incompleteSoloStreaks, defaultCurrentStreak, endDate);
    });
});